      ...
      <v-card elevation="1">
        ...
        <form @submit.prevent="submit">
          <v-card-text>
            ...
            <v-text-field outlined v-model="ra" :error-messages="raErrors" :counter="140" label="RA"
              placeholder="Informe o registro acadêmico" required @input="$v.ra.$touch()" @blur="$v.ra.$touch()" />

            <v-text-field outlined v-model="cpf" :error-messages="cpfErrors" :counter="14" label="CPF"
  ...
  methods: {
    async submit() {
      try {
        ...
        this.showAlert = true;
        this.alertText = 'Aluno criado com sucesso!';
        this.alertType = 'success';
      } catch (error) {
        this.showAlert = true;
        this.alertText = 'Erro ao criar o aluno!';
